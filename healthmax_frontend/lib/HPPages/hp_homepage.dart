...
class _HPHomePageState extends State<HPHomePage> {
  ...
  Widget build(BuildContext context) {
    final currentColor = metricColors[selectedMetric] ?? const Color(0xFF8E33FF);
        ...
        children: [
          ...
          Positioned(
            ...
            child: Column(
              ...
              children: const [
                Text("Hi,", style: TextStyle(fontSize: 55, fontWeight: FontWeight.bold, color: Colors.white, height: 1.0)),
                Text("Hospital 1.", style: TextStyle(fontSize: 55, fontWeight: FontWeight.bold, color: Colors.white, height: 1.0)),
              ],
            ),
          ),
                        ...
                        SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text("$selectedMetric Trends", 
                                style: TextStyle(fontSize: 30, color: currentColor, fontWeight: FontWeight.bold, letterSpacing: -0.5)),
    ...
    return Row(
      children: [
        Expanded(
          child: Column(
            children: [
              _statBtn(Icons.person, "10 Connected Users", const Color.fromARGB(255, 158, 243, 202), 
                onTap: () => Navigator.pushReplacementNamed(context, '/users')),
              const SizedBox(height: 12),
              _statBtn(Icons.access_time, "5 Requests", const Color.fromARGB(255, 248, 194, 124), 
                onTap: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const HPRequestsPage()))),
            ],
          ),
        ),
        const SizedBox(width: 15),
        _statBtn(Icons.warning, "3 NEED ATTENTION", const Color.fromARGB(255, 251, 51, 51), isLarge: true, textColor: Colors.white),
      ],
    );
  }

  // INDIVIDUAL STAT BUTTON BUILDER
Widget _statBtn(IconData icon, String text, Color color, {bool isLarge = false, VoidCallback? onTap, Color textColor = Colors.black, }) { // default text is black
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              _dropdown(['Heart Rate', 'Steps', 'Glucose Level', 'Calories'], selectedMetric, (v) => setState(() => selectedMetric = v!)),
              _dropdown(['Week', 'Month', 'Year'], selectedTimeframe, (v) => setState(() => selectedTimeframe = v!)),
            ],
          ),
          const SizedBox(height: 20),
  ...
  Widget _actionBtn(String label, Color col, IconData icon) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: col, 
        borderRadius: BorderRadius.circular(20),
        boxShadow: [BoxShadow(color: col.withOpacity(0.4), blurRadius: 10, offset: const Offset(0, 4))],
      ),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
  ...
  Widget _buildAlertTile(String name, String issue, String value, Color bg) {
    return Container(
      ...
      child: Row(
        children: [
          const CircleAvatar(backgroundColor: Colors.white, radius: 15, child: Icon(Icons.person, size: 15, color: Colors.black)),
          const SizedBox(width: 15),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                Text(issue, style: const TextStyle(fontSize: 12, color: Colors.black54)),
              ],
            ),
          ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 15),
          const Expanded(
            child: Text("All hospital systems are operational. Device sync is at 98% efficiency.", 
  ...
  Widget _dropdown(List<String> items, String val, ValueChanged<String?> onChanged) {
  ...
  LineChartData _sampleChartData(Color color) {
    return LineChartData(
      ...
      lineBarsData: [
        LineChartBarData(
          spots: const [FlSpot(0, 3), FlSpot(2, 4), FlSpot(4, 3.5), FlSpot(6, 5), FlSpot(8, 4.2), FlSpot(10, 6)],
          isCurved: true, color: color, barWidth: 4, dotData: const FlDotData(show: false),
        ),
      ],
    );
  }
